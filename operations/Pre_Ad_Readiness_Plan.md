...
### Preferred carousel requirement — pending Bảo's format sign-off

Carousel is the preferred LinkedIn awareness format under consideration. If Bảo selects it, the approved static concepts remain available and the asset scope expands by **four additional square carousel cards**. Their role is to establish Digiwin's semiconductor credibility before asking for a discussion; they are not performance, customer-outcome or lead-generation claims.
...
3. **Representative customers:** approved customer examples relevant to semiconductor operations, only where name, logo, scope, date and advertising usage rights are explicitly cleared.
...
Each card must carry a recorded source, owner, geography, date/scope, claim wording and usage-right decision before production. Do not use customer names/logos, market position, deployment scale, results, customer outcomes or cross-market extrapolation unless that exact use is verified. If any proof remains incomplete, replace it with a mechanism-only statement or keep the card out of the carousel. No carousel image is generated by this plan amendment.
