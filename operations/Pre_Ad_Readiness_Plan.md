...
### Preferred carousel requirement — Bảo-approved offline v1

Carousel is the Bảo-approved LinkedIn awareness format for the offline v1 creative pack. The approved static concepts remain available and the asset scope includes **four additional square carousel cards**. Their role is to establish Digiwin's semiconductor credibility before asking for a discussion; they are not performance, customer-outcome or lead-generation claims.

1. **Taiwan semiconductor capability:** bounded, source-cleared evidence of Digiwin's relevant semiconductor experience or delivery capability in Taiwan.
2. **China semiconductor capability:** bounded, source-cleared evidence of Digiwin's relevant semiconductor experience or delivery capability in China.
3. **Representative customers:** not selected for offline v1; future named examples are allowed only where name, logo, scope, date and advertising usage rights are explicitly cleared.
4. **Vietnam localization:** verified evidence of Digiwin's Vietnam-local team, service or delivery capability, stated only within the confirmed local scope.

Each card carries a recorded source, owner, geography, date/scope, claim wording and usage-right decision in `operations/LinkedIn_Carousel_Governance_Record.md` and the linked pre-generation brief. Bảo approved the exact offline v1 claim/metric copy; customer names and logos remain unapproved, so CAR-03 uses an abstract proof-safe replacement. Do not use market position, deployment scale, results, customer outcomes or cross-market extrapolation beyond the recorded scope. The four offline v1 PNGs are canonical under `assets/linkedin/final/`; final account-time export size, LinkedIn object creation, upload, launch and spend remain separate gates.
