...
**Current Planner evidence (2026-09-14, Asia/Ho_Chi_Minh):** screenshots showed no ad-blocker dialog. The string remained in the accessibility/DOM tree, but direct navigation and the Keyword Planner query form worked; classify `OBSERVED_DOM_FALSE_SIGNAL_NON_BLOCKING`. The orange Vietnamese goal/budget mechanism-update banner was visible and informational. Query configuration was Vietnam, Vietnamese, Google, Sep 2025–Aug 2026, with adult ideas excluded. For the OSAT lot/test/traceability seed cluster, all displayed metric fields were dashes and the chart/related-idea panel showed no data. Record `OBSERVED_NO_DISPLAYED_DATA_IN_CURRENT_CONFIGURATION`; do not infer zero demand. The query generated one draft plan entry automatically despite no explicit save click. Further queries were stopped under the no-save constraint. No campaign was created or modified, no settings changed, and no enablement or spend occurred. This is not a validated account-wide failure. Geography, language, network and auto-tagging for any campaign remain unverified.
